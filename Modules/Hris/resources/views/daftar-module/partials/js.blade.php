...
$(function () {
    ...
    var moduleTable = $('#module-table').DataTable({
        serverSide: true,
        processing: false,
        ajax: "{{ route('hris.daftar-module.index') }}",
        columns: [
            {data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false},
            {data: 'title', name: 'title'},
            {data: 'url', name: 'url'},
            {data: 'icon', name: 'icon', render: function(data) {
                return `<i class="${data} fs-20"></i> (${data})`;
            }},
    ...
    $('#form-module').on('submit', function(e) {
        ...
        let url = id ? "{{ route('hris.daftar-module.update', ':id') }}".replace(':id', id) : "{{ route('hris.daftar-module.store') }}";
    ...
    $('#btn-export-excel, #btn-export-pdf').on('click', function () {
        ...
        let baseUrl = isExcel ? "{{ route('hris.daftar-module.excel') }}" : "{{ route('hris.daftar-module.pdf') }}";
...
function showDetail(id) {
    ...
    $.get("{{ route('hris.daftar-module.show', ':id') }}".replace(':id', id), function(response) {
        ...
        if(response.success) {
            ...
            // 1. Setup Modal
            $('#modal-title').text('Detail Module');
            ...
            $('#url').val(data.url);
            $('#module_status').val(data.status);
            $('#icon-input').val(data.icon);
            $('#icon-preview').html(`<i class="${data.icon}"></i>`);
...
function addMode() {
    ...
    $('#icon-input').val('');
    $('#module_status').val('');
    $('#icon-preview').html('<i class="ri-search-line"></i>');
    $('#showModal').modal('show');
}
...
function confirmDelete(id, title) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "{{ route('hris.daftar-module.destroy', ':id') }}".replace(':id', id),
                type: "DELETE",
                data: { 
                    _token: "{{ csrf_token() }}" 
                },
